...
export class PieChartAgent extends BaseTravelAgent {
  ...
  async generateChart(request: TravelAgentRequest): Promise<AgentResponse> {
    try {
      // Check if this request has filter requirements
      const hasFieldFilter = request.constraints?.hasFieldFilter === true;
      const filterConfig = request.constraints?.filterConfig;
...
${hasFieldFilter ? `
2. FILTER CONFIGURATION DETECTED:
   - Filter Type: ${filterConfig?.filterType || 'unknown'}
   - Filter Key: ${filterConfig?.filterKey || 'unknown'}
   - Generate data for default filter value: ${filterConfig?.options?.[0]?.value || 'first option'}
   - Data variations will be generated by a second-layer agent
